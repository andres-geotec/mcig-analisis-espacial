...
formato_clave <- function(clave, n_caracteres_cve) {
  return(substr(str_pad(as.character(clave),
                        width = n_caracteres_cve,
                        pad = "0",
                        side = "left"), 1, n_caracteres_cve))
}

leer_movilidad <- function(archivo, n_caracteres_cve) {
  return(read.csv(archivo) |> mutate(
      # asegurar los caracteres adecuados con ceros a la izquierda
      source = formato_clave(source, n_caracteres_cve),
      target = formato_clave(target, n_caracteres_cve)))
}

en_yucatan <- function(clave) {
  return(str_starts(clave, paste(zm_muns, collapse = "|")))
}
...
guardar_movilidad <- function(granularidad, dfs) {
  for (tipo in names(dfs)) {
    nombre <- paste0(granularidad, "-", tipo, ".csv")
    write.csv(dfs[[tipo]], 
              file.path(data_procesada, nombre),
              row.names = FALSE)
  }
}
...
#' preparar_movilidad(granularidad = "ageb")
preparar_movilidad <- function(granularidad = "municipio") {
  # archivos disponibles de movilidad
  archivos <- list.files(
    path = file.path(data_original, paste0(
        "movilidad_",
        ifelse(granularidad == "localidad", "ageb", granularidad))),
  ...
  n_caracteres_cve <- list(municipio = 5, localidad = 9, ageb = 13)[[granularidad]]
  for(archivo in archivos) {
    df <- leer_movilidad(archivo, n_caracteres_cve)
    
    # Filtra los que solo se movinron dentro de Yucatán
    internos <- df |> filter(en_yucatan(source) & en_yucatan(target))
    ...
    llegadas <- df |> filter(!en_yucatan(source) & en_yucatan(target))
    ...
    salidas <- df |> filter(en_yucatan(source) & !en_yucatan(target))
  ...
  guardar_movilidad(granularidad, list(interno = internos_acumulado,
                                       llegadas = llegadas_acumulado,
                                       salidas = salidas_acumulado,
...
estadisticas_ageb <- preparar_movilidad("ageb")
summary(estadisticas_ageb)
```

```{r Acumulados por localidad, eval=FALSE}
estadisticas_loc <- preparar_movilidad("localidad")
summary(estadisticas_loc)
```
